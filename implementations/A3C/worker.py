...
class Worker(object):
  def __init__(self, name, env, model_net, discount_factor, t_max, global_counter, summary_writer):
    ...
    self.av_reward_pl = tf.placeholder(shape=(), dtype=tf.float32)
    ...
    self.summaries_op = tf.summary.merge([
        ...
        tf.summary.scalar("mean reward", self.av_reward_pl)
    ])
  ...
  def run_n_steps(self, sess):
      ...
      for _ in range(self.t_max):
          ...
          if done:
              ...
              print("agent {}, episode {}, total_reward {}, episode_length {}, mean_reward {}, action distr {}".format(
                  self.name, self.episode_counter, int(self.total_reward*100), self.episode_length, np.mean(self.episode_rewards[-5:]),np.bincount(self.action_counter)))

              summaries = sess.run(self.summaries_op, {self.ep_reward_pl: int(self.total_reward * 100),
                                                       self.ep_length_pl: self.episode_length,
                                                       self.av_reward_pl: np.mean(self.episode_rewards[-5:])})
